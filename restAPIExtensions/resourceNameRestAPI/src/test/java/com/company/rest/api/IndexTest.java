...
class IndexTest {
    ...
    void should_get_result_when_params_ok() {
        ...
        // Given
        String sortBy = "aValue1";
        String sortType = "aValue2";

        // When
        Result result = index.execute(context, sortBy, sortType);

        // Then
        assertThat(result.getSortBy()).isEqualTo(sortBy);
        assertThat(result.getSortType()).isEqualTo(sortType);
    ...
    void should_return_a_json_representation_as_result() throws IOException {
        ...
        when(httpRequest.getParameter("sortBy")).thenReturn("aValue1");
        when(httpRequest.getParameter("sortType")).thenReturn("aValue2");
    ...
    @Test
    void should_return_an_error_response_if_sortBy_is_not_set() throws IOException {
        // Given "a request without sortBy"
        when(httpRequest.getParameter("sortBy")).thenReturn(null);
        when(httpRequest.getParameter("sortType")).thenReturn("aValue2");
        ...
        assertThat(jsonResponse.getMessage()).isEqualTo("the parameter sortBy is missing");
    }

    @Test
    void should_return_an_error_response_if_sortType_is_not_set() throws IOException {
        // Given "a request without sortType"
        when(httpRequest.getParameter("sortType")).thenReturn(null);
        when(httpRequest.getParameter("sortBy")).thenReturn("aValue1");
        ...
        // Validate returned response
        assertThat(apiResponse.getHttpStatus()).isEqualTo(400);
        assertThat(jsonResponse.getMessage()).isEqualTo("the parameter sortType is missing");
    }
